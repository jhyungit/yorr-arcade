...
import { feedbackDiceLand, unlockAudio, vibrate } from '../../lib/feedback'
...
export default function Controller({ initialCode }: ControllerProps) {
  ...
  useEffect(() => {
    ...
    const onHit = (d?: { player?: number; kind?: string }) => {
      ...
      // vibrate() 가 설정·기기 지원을 다 판단하고 실제로 울렸는지 알려준다.
      // (예전엔 navigator.vibrate 를 직접 불러서 설정에서 진동을 꺼도 울렸다)
      const p =
        d?.kind === 'dice'
          ? [0, 24, 30, 46]
          : d?.kind === 'smash'
            ...
            : d?.kind === 'foul'
              ? [0, 90, 60, 90, 60, 90]
              : 35
      // 주사위 착지는 진동이 안 될 때(아이폰) 소리로 대체해 알려준다
      if (!vibrate(p) && d?.kind === 'dice') feedbackDiceLand()
    }
  ...
  return (
    ...
    >
      ...
      {/* 설정 — 소리·진동. 값은 lib/feedback 이 들고 있어 노트북 화면과 같은 스위치다 */}
      <SettingsGear className="absolute right-3 top-3 z-[60]" />
