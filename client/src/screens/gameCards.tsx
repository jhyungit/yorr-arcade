 ...
 *
 * 아트는 전부 div/그라디언트/clip-path 로만 그린다 — 외부 이미지·폰트 금지
...
function Die({ n, style }: { n: number; style: React.CSSProperties }) {
  const on = new Set(PIPS[n])
  return (
    <div
      className="absolute rounded-[22%] bg-gradient-to-br from-[#fffdf7] to-[#e6ddc9]"
...
function YachtArt() {
  return (
    ...
    >
      ...
      />
      <Die n={5} style={{ left: '14%', top: '38%', width: '23%', aspectRatio: '1', rotate: '-15deg' }} />
      <Die n={3} style={{ left: '39%', top: '26%', width: '26%', aspectRatio: '1', rotate: '8deg' }} />
      <Die n={6} style={{ left: '65%', top: '42%', width: '22%', aspectRatio: '1', rotate: '-6deg' }} />
      {/* 바닥 그림자 */}
      <div
  ...
  return (
    <svg viewBox="0 0 160 90" className="absolute inset-0 h-full w-full" preserveAspectRatio="none">
      <defs>
        ...
        <radialGradient id="pp-spot" cx="0.5" cy="0.3" r="0.6">
      ...
      <rect width="160" height="90" fill="url(#pp-spot)" />
      {/* 상판 — 사다리꼴(원근). 흰 테두리는 stroke 로. */}
      <polygon points="50,26 110,26 158,90 2,90" fill="url(#pp-top)" stroke="#f4f8fb" strokeWidth="1.4" />
      {/* 센터라인 (대칭 사다리꼴이라 수직) */}
      ...
      <ellipse cx="30" cy="72" rx="9" ry="8" fill="#2b8fe0" />
      <ellipse cx="105" cy="30" rx="5.5" ry="5" fill="#e2513c" />
    </svg>
  )
}
...
function RhythmArt() {
  return (
    <div className="absolute inset-0 overflow-hidden" style={{ background: '#060309' }}>
      {LANES.map((c, i) => {
        ...
        const bl = i * 25
        const br = (i + 1) * 25
        // 노트도 그 높이에서의 레인 폭을 따라야 원근이 맞는다 (안 그러면 위쪽에서 레인 밖으로 삐져나온다)
        const t = 0.3 + i * 0.15 // 레인마다 다른 높이에 하나씩
        const nl = tl + (bl - tl) * t
        const nr = tr + (br - tr) * t
...
function Gunman({ x, flip }: { x: number; flip?: boolean }) {
  return (
    <g transform={`translate(${x} 0)${flip ? ' scale(-1 1)' : ''}`} fill="#120a08">
      {/* 모자 (크라운 + 챙) */}
...
function ReactionArt() {
  return (
    <svg viewBox="0 0 160 90" className="absolute inset-0 h-full w-full" preserveAspectRatio="none">
      ...
      <rect width="160" height="90" fill="url(#qd-sky)" />
      <circle cx="80" cy="62" r="19" fill="url(#qd-sun)" />
      <rect y="62" width="160" height="28" fill="url(#qd-ground)" />
      <Gunman x={28} />
      <Gunman x={132} flip />
      {/* 신호등 — 매달려 있고 지금 막 초록 */}
      <rect x="79.2" y="0" width="1.6" height="9" fill="#2a1a14" />
      <rect x="75" y="9" width="10" height="20" rx="2.5" fill="#1a1210" />
      <circle cx="80" cy="13.5" r="2.2" fill="#7a1f14" />
      <circle cx="80" cy="19" r="2.2" fill="#6b4a24" />
      <circle cx="80" cy="24.5" r="2.6" fill="#4ade80" />
      <circle cx="80" cy="24.5" r="5" fill="#4ade80" opacity="0.28" />
    </svg>
  )
}
...
function SlasherArt() {
  ...
  return (
    <div className="absolute inset-0 overflow-hidden" style={{ background: '#070a18' }}>
      {/* 격자 */}
      <div
      ...
      {chips.map((c) => (
        <div
          key={c.t}
          className="absolute flex items-center justify-center rounded-lg border border-white/15 bg-white/[0.07] text-[10px] font-black text-white/70"
