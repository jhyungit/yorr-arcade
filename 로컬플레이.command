...
echo "=================================================================="
if [ -z "$IPS" ]; then
  echo " ‼️ 사설 IP 가 없습니다 — 핫스팟/와이파이 연결을 확인하세요."
  echo "    (이 상태로는 폰에서 접속할 수 없습니다)"
  echo
fi
echo " 잠시 뒤 아래에 \"접속 주소\" 한 줄이 뜹니다."
echo "   맥 브라우저도 폰 브라우저도 그 주소 하나로 들어가세요."
echo "   (맥을 localhost 로 열면 QR 이 폰에서 자기 자신을 가리켜 연결이 안 됩니다)"
echo
echo " * 인증서 경고가 뜨면 '고급 -> 계속 진행'(아이폰은 '웹사이트 방문')"
echo " * 맥 화면에서 [폰 연결] -> QR 을 폰 카메라로 찍으면 컨트롤러가 됩니다"
